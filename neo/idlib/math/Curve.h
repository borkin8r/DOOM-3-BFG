...
*/
template< class type >
ID_INLINE float idCurve_BSpline<type>::Basis( const int index, const int anOrder, const float t ) const {
	if ( anOrder <= 1 ) {
		...
		float sum = 0.0f;
		float d1 = this->TimeForIndex( index+anOrder-1 ) - this->TimeForIndex( index );
		if ( d1 != 0.0f ) {
			sum += (float) ( t - this->TimeForIndex( index ) ) * Basis( index, anOrder-1, t ) / d1;
		}

		float d2 = this->TimeForIndex( index+anOrder ) - this->TimeForIndex( index+1 );
		if ( d2 != 0.0f ) {
			sum += (float) ( this->TimeForIndex( index+anOrder ) - t ) * Basis( index+1, anOrder-1, t ) / d2;
		}
		return sum;
	}
}
...
*/
template< class type >
ID_INLINE float idCurve_BSpline<type>::BasisFirstDerivative( const int index, const int anOrder, const float t ) const {
	return ( Basis( index, anOrder-1, t ) - Basis( index+1, anOrder-1, t ) ) *
			(float) ( anOrder - 1 ) / ( this->TimeForIndex( index + ( anOrder - 1 ) - 2 ) - this->TimeForIndex( index - 2 ) );
}
...
*/
template< class type >
ID_INLINE float idCurve_BSpline<type>::BasisSecondDerivative( const int index, const int anOrder, const float t ) const {
	return ( BasisFirstDerivative( index, anOrder-1, t ) - BasisFirstDerivative( index+1, anOrder-1, t ) ) *
			(float) ( anOrder - 1 ) / ( this->TimeForIndex( index + ( anOrder - 1 ) - 2 ) - this->TimeForIndex( index - 2 ) );
}
...
*/
template< class type >
ID_INLINE void idCurve_NonUniformBSpline<type>::Basis( const int index, const int anOrder, const float t, float *bvals ) const {
    int r, s, i;
    float omega;

    bvals[anOrder-1] = 1.0f;
    for ( r = 2; r <= anOrder; r++ ) {
		i = index - r + 1;
		bvals[anOrder - r] = 0.0f;
		for ( s = anOrder - r + 1; s < anOrder; s++ ) {
			i++;
...
*/
template< class type >
ID_INLINE void idCurve_NonUniformBSpline<type>::BasisFirstDerivative( const int index, const int anOrder, const float t, float *bvals ) const {
	int i;

	Basis( index, anOrder-1, t, bvals+1 );
	bvals[0] = 0.0f;
	for ( i = 0; i < anOrder-1; i++ ) {
		bvals[i] -= bvals[i+1];
		bvals[i] *= (float) ( anOrder - 1) / ( this->TimeForIndex( index + i + (anOrder-1) - 2 ) - this->TimeForIndex( index + i - 2 ) );
	}
	bvals[i] *= (float) ( anOrder - 1) / ( this->TimeForIndex( index + i + (anOrder-1) - 2 ) - this->TimeForIndex( index + i - 2 ) );
}
...
*/
template< class type >
ID_INLINE void idCurve_NonUniformBSpline<type>::BasisSecondDerivative( const int index, const int anOrder, const float t, float *bvals ) const {
	int i;

	BasisFirstDerivative( index, anOrder-1, t, bvals+1 );
	bvals[0] = 0.0f;
	for ( i = 0; i < anOrder-1; i++ ) {
		bvals[i] -= bvals[i+1];
		bvals[i] *= (float) ( anOrder - 1) / ( this->TimeForIndex( index + i + (anOrder-1) - 2 ) - this->TimeForIndex( index + i - 2 ) );
